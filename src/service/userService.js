...
const authService = {
    svCreate: async (req, res) => {
        ...
        try {
            ...
            await querySQl(
                `INSERT INTO ${constant.TABLE_DATABASE.USER_ROLE} (roleID, userID, createdBy)
                 VALUES (?, ?, ?)`,
                [roleDB[0]['roleID'], userID, payload.createdBy]
            );
    ...
    svUpdate: async (req, res) => {
        ...
        try {
            ...
            await querySQl(
                `UPDATE ${constant.TABLE_DATABASE.USER_ROLE}
                 SET roleID    = ?,
                     updatedBy = ?
                 WHERE userID = ?`,
    ...
    svGetByID: async (req, res) => {
        ...
        try {
            let userDB = await querySQl(`SELECT u.*, r.roleID, r.name as roleName, c.name as companyName
                                         FROM ${constant.TABLE_DATABASE.USER} AS u
                                                  LEFT JOIN ${constant.TABLE_DATABASE.USER_ROLE} AS ur
                                                            ON u.userID = ur.userID
                                                  LEFT JOIN ${constant.TABLE_DATABASE.COMPANY} AS c
            ...
            console.error('Error executing query get by id user :', err.stack);
            return res
                .status(constant.SYSTEM_HTTP_STATUS.INTERNAL_SERVER_ERROR)
                .json({
                    status: constant.SYSTEM_HTTP_STATUS.INTERNAL_SERVER_ERROR,
