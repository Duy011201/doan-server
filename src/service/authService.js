...
const authService = {
  svLogin: async (req, res) => {
    ...
    try {
      ...
      if (
        isEmpty(userDB) ||
        !(await bcryptComparePassword(payload.password, userDB[0][`password`]))
      )
        return res.status(constant.SYSTEM_HTTP_STATUS.BAD_REQUEST).json({
          status: constant.SYSTEM_HTTP_STATUS.BAD_REQUEST,
      ...
      userDB = filterFields(userDB, ['userID', 'email', 'status', 'avatar', 'companyID']);
      ...
      return res.status(constant.SYSTEM_HTTP_STATUS.OK).json({
        ...
        message: constant.RESPONSE_MESSAGE.SUCCESS_LOGIN_ACCOUNT,
        data: userDB[0],
      });
    } catch (err) {
  ...
  svRegister: async (req, res) => {
    ...
    try {
      ...
      if (payload.role === constant.SYSTEM_ROLE.EMPLOYER) {
        ...
        if (!isEmpty(companyDB))
          return res.status(constant.SYSTEM_HTTP_STATUS.BAD_REQUEST).json({
            status: constant.SYSTEM_HTTP_STATUS.BAD_REQUEST,
            massage:
              constant.RESPONSE_MESSAGE.ERROR_COMPANY_CORPORATE_TAX_CODE_EXIT,
          });

        await querySQl(
          `INSERT INTO ${constant.TABLE_DATABASE.COMPANY} (companyID, name, corporateTaxCode)
                                VALUES (?, ?, ?)`,
          [companyID, payload.companyName, payload.companyCorporateTaxCode]
        );

        await querySQl(
          `INSERT INTO ${constant.TABLE_DATABASE.USER} (userID, email, password, status, companyID)
                                VALUES (?, ?, ?, ?, ?)`,
          [
            userID,
            payload.email,
      ...
      if (payload.role === constant.SYSTEM_ROLE.CANDIDATE)
        await querySQl(
          `INSERT INTO ${constant.TABLE_DATABASE.USER} (userID, email, password, status)
                                VALUES (?, ?, ?, ?)`,
          [userID, payload.email, hashPassword, constant.SYSTEM_STATUS.ACTIVE]
        );

      let roleDB = await querySQl(
        `SELECT r.roleID
      ...
      await querySQl(
        `INSERT INTO ${constant.TABLE_DATABASE.USER_ROLE} (roleID, userID)
                            VALUES (?, ?)`,
        [roleDB[0]['roleID'], userID]
      );

      await querySQl(
        `UPDATE ${constant.TABLE_DATABASE.VERIFY_CODE}
                            SET status = ?
                            WHERE email = ?`,
  ...
  svVerifyCode: async (req, res) => {
    ...
    try {
      ...
      if (isEmpty(verifyCodeDB)) {
        await querySQl(
          `INSERT INTO ${constant.TABLE_DATABASE.VERIFY_CODE} (verifyCodeID, code, email)
                                VALUES (?, ?, ?)`,
          [verifyCodeID, verifyCode, payload.email]
        );
      } else {
        await querySQl(
          `UPDATE ${constant.TABLE_DATABASE.VERIFY_CODE}
  ...
  svForgotPassword: async (req, res) => {
    ...
    try {
      ...
      await querySQl(
        `UPDATE ${constant.TABLE_DATABASE.USER}
                            SET password  = ?,
                                updatedBy = ?
                            WHERE email = ?`,
