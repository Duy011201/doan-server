...
const authService = {
    ...
    * check email , status, password, role
    ...
    svLogin: async (req, res) => {
        ...
        try {
            let usersDB = await performSQL(setting.SQL_METHOD.GET, setting.TABLE_DATABASE.USER, [{email: email}]);
            switch (true) {
            ...
            let roles = await performSQL(setting.SQL_METHOD.GET, setting.TABLE_DATABASE.ROLE, [{userID: usersDB[0][`userID`]}]);
        ...
        try {
            ...
            if (isEmpty(verifyCodeDB)) {
                await performSQL(setting.SQL_METHOD.INSERT, setting.TABLE_DATABASE.VERIFY_CODE,
                    [{verifyCodeID: verifyCodeID}, {code: verifyCode}, {email: email}, {createdBy: createdBy}, {updatedBy: updatedBy}])
            }

            await performSQL(setting.SQL_METHOD.UPDATE, setting.TABLE_DATABASE.VERIFY_CODE, [{code: verifyCode},
