...
const authService = {
  svLogin: async (req, res) => {
    ...
    if (!isEmail(email)) {
      return res
        ...
        .json({ message: setting.RESPONSE_MESSAGE.INVALID_EMAIL_FORMAT });
    }
    if (isEmpty(password)) {
      return res
        .status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST)
        .json({ message: setting.RESPONSE_MESSAGE.INVALID_PASSWORD_FORMAT });
    }

    try {
      let usersDB = await performSQL(
      ...
      switch (true) {
        case usersDB && usersDB.length > 1:
          return res
            ...
            .json({ message: setting.RESPONSE_MESSAGE.ERROR_EXIT_ANY_ACCOUNT });
        case isEmpty(usersDB) ||
          !(await bcryptComparePassword(password, usersDB[0][`_password`])):
      ...
      let roles = await performSQL(
        setting.SQL_METHOD.GET,
        setting.TABLE_DATABASE.ROLE,
        [{ _userID: usersDB[0][`_userID`] }]
      );
      if (isEmpty(roles)) {
        return res
          .status(setting.SYSTEM_STATUS_CODE.UNAUTHORIZED)
          .json({ message: setting.RESPONSE_MESSAGE.ERROR_ROLE });
      }
  ...
  svVerifyCode: async (req, res) => {
    ...
    if (!isEmail(email)) {
      return res
        .status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST)
        .json({ message: setting.SYSTEM_STATUS_MESSAGE.INVALID_EMAIL_FORMAT });
    }
    ...
    try {
      ...
      if (isEmpty(verifyCodeDB)) {
        await querySQl(
          `INSERT INTO ${setting.TABLE_DATABASE.VERIFY_CODE} (_verifyCodeID, _code, _email, _status, _createdBy, _updatedBy)
                                VALUES (?, ?, ?, ?, ?, ?)`,
        ...
        await performSQL(
          ...
          [
            { _code: verifyCode },
            { _status: setting.SYSTEM_STATUS.ACTIVE },
            { _email: email },
      ...
      return res
        ...
        .json({ message: setting.RESPONSE_MESSAGE.SUCCESS_SEND_VERIFY_CODE });
    } catch (err) {
  ...
  svForgotPassword: async (req, res) => {
    ...
    try {
      ...
      let hashPassword = await bcryptHashPassword(password);
      await performSQL(setting.SQL_METHOD.UPDATE, setting.TABLE_DATABASE.USER, [
        { _password: hashPassword },
        { _updatedBy: userDB[0]['_userID'] },
        { _userID: userDB[0]['_userID'] },
      ]);
      ...
      return res
        .status(setting.SYSTEM_STATUS_CODE.OK)
        .json({ message: setting.RESPONSE_MESSAGE.SUCCESS_FORGOT_PASSWORD });
    } catch (err) {
