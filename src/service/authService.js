...
const authService = {
    ...
    svRegister: async (req, res) => {
        ...
        try {
            ...
            if (!isEmpty(users)) {
                return res.status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST).json({ message: setting.SYSTEM_STATUS_MESSAGE.ERROR_EMAIL_ALREADY_EXIT });
            }

            const verifyCodeDB = await performSQL(setting.SQL_METHOD.GET, setting.TABLE_DATABASE.VERIFY_CODE, [{_email: email}]);

            // Default one day
            if (!isEmpty(verifyCodeDB) && !isEmpty(role) && !isEmpty(desc) && timeDiff(today, verifyCodeDB[0]['_updatedAt'], 1) && verifyCodeDB[0]['_code'] === verifyCode) {
                ...
                await querySQl(`INSERT INTO ${setting.TABLE_DATABASE.ROLE} (_roleID, _userID, _name, _desc, _createdBy, _updatedBy)
    ...
    svVerifyCode: async (req, res) => {
        ...
        try {
            ...
            if (isEmpty(verifyCodeDB)) {
                await querySQl(`INSERT INTO ${setting.TABLE_DATABASE.VERIFY_CODE} (_verifyCodeID, _code, _email, _createdBy, _updatedBy)
            ...
            return res.status(setting.SYSTEM_STATUS_CODE.OK).json({ message: setting.RESPONSE_MESSAGE.SUCCESS_SEND_VERIFY_CODE, verifyCode: verifyCode });
        } catch (err) {
    ...
    svForgotPassword: async (req, res) => {
        ...
        if (isEmpty(verifyCode) || isEmpty(password)) {
            return res.status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST).json({ message: setting.SYSTEM_STATUS_MESSAGE.INVALID_REQUEST_PARAMETERS });
        }
        if (!isEmail(email)) {
            return res.status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST).json({ message: setting.SYSTEM_STATUS_MESSAGE.INVALID_EMAIL_FORMAT });
        }
        ...
        try {
            ...
            return res.status(setting.SYSTEM_STATUS_CODE.BAD_REQUEST).json({ message: setting.RESPONSE_MESSAGE.ERROR_FORGOT_PASSWORD });
        } catch (err) {
    ...
    svRefreshToken: async (req, res) => {
        return res.status(setting.SYSTEM_STATUS_CODE.OK).json({ message: setting.SYSTEM_STATUS_MESSAGE.SUCCESS_REFRESH_TOKEN, token: refreshToken(req.body.token) });
    },
}
