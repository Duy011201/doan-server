    ...
    login: async (req, res) => {
        ...
        try {
            await query(`SELECT *
                     ...
                     WHERE user._email = ? and user._password = ?`, [email, password]).then(results => {
                // if (isPassword(password, )) {
                //     return res.json(new HttpException(setting.SYSTEM_STATUS_CODE.BadRequest, setting.ERROR_MESSAGE.INVALID_EMAIL_FORMAT, []));
                // }

                return res.json(new HttpException(setting.SYSTEM_STATUS_CODE.OK, setting.RESPONSE_MESSAGE.REGISTER_ACCOUNT_SUCCESS, {}));
            })
        } catch (err) {
